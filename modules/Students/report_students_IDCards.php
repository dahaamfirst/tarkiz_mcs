    ...
    if (count($choices) > 0) {
        ...
        if ($result->rowCount() < 1) {
            ...
            while ($row = $result->fetch()) {
                ...
                echo "<td style='width:".(100 / $columns)."%; text-align: center; vertical-align: center'>";
                echo "<div style='width: 488px; height: 308px; border: 1px solid black; $bg'>";
                echo "<table class='blank' cellspacing='0' style='width 448px; max-width 448px; height: 268px; max-height: 268px; margin: 85px 10px 10px 10px'>";
                echo '<tr>';
                echo "<td style='padding: 0px ; width: 150px; height: 150px; vertical-align: center'>";
                if ($row['image_240'] == '' or file_exists($session->get('absolutePath').'/'.$row['image_240']) == false) {
                    echo "<img style='width: 150px; height: 150px' class='user' src='".$session->get('absoluteURL').'/themes/'.$session->get('gibbonThemeName')."/img/anonymous_240.jpg'/><br/>";
                } else {
                    echo "<img style='width: 150px; height: 150px' class='user' src='".$session->get('absoluteURL').'/'.$row['image_240']."'/><br/>";
                }
                echo '</td>';
                echo "<td style='padding: 0px ; width: 18px'></td>";
                // Modified for RTL layout
                echo "<td style='padding: 0px 0 0 0 ; text-align: right; width: 280px; vertical-align: top; font-size: 16px; direction: rtl;'>";
                echo "<div style='padding: 7px; background-color: rgba(255,255,255,0.3); min-height: 200px'>";
                ...
                // Name with RTL alignment
                echo "<div style='font-weight: bold; font-size: ".$size."px !important;'>".$row['officialName'].'</div>';
                
                // Student details with RTL labels and proper formatting
                echo '<div dir="rtl">';
                echo '<b>'.__('DOB').":</b> <span dir='ltr'>".Format::date($row['dob']).'</span><br/>';
                echo '<b>'.$session->get('organisationNameShort').' '.__('ID').":</b> <span dir='ltr'>".$row['studentID'].'</span><br/>';
                echo '<b>'.__('Year/Form').":</b> <span dir='ltr'>".__($row['year']).' / '.$row['form'].'</span><br/>';
                echo '<b>'.__('School Year').":</b> <span dir='ltr'>".$session->get('gibbonSchoolYearName').'</span><br/>';
                echo '</div>';
                echo '</div>';
                echo '</td>';
                echo '</tr>';
