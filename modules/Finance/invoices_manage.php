        ...
        $table->addActionColumn()
            ...
            ->format(function ($invoice, $actions) {
                ...
                if ($invoice['status'] == 'Pending') {
                    ...
                    // ضمن حلقة عرض كل فاتورة $row
                    if (($row['status'] ?? '') === 'Issued') {
                        // Nonce بسيط ضد CSRF (لأننا داخل تعديل يدوي للنواة)
                        if (empty($_SESSION['waSendNonce'])) {
                            $_SESSION['waSendNonce'] = bin2hex(random_bytes(16));
                        }
                    
                        // نموذج إرسال POST لسكربت المعالجة الذي سننشئه الآن
                        echo '<form method="post" action="index.php?q=/modules/Finance/invoices_manage_sendWhatsApp.php" style="display:inline-block;margin-inline-start:.25rem">';
                        echo '<input type="hidden" name="gibbonFinanceInvoiceID" value="'.(int)$row['gibbonFinanceInvoiceID'].'">';
                        echo '<input type="hidden" name="nonce" value="'.htmlspecialchars($_SESSION['waSendNonce']).'">';
                        echo '<button type="submit" class="btn btn-secondary">إرسال عبر واتساب</button>';
                        echo '</form>';
                    }
